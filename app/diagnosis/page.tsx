...
const compressAvatarImage = async (file: File): Promise<File> => {
  ...
  const initialScale = longestEdge > AVATAR_MAX_DIMENSION ? AVATAR_MAX_DIMENSION / longestEdge : 1;
  ...
  if (bestBlob && bestBlob.size <= AVATAR_MAX_UPLOAD_BYTES) {
    return new File([bestBlob], file.name.replace(/\.[^/.]+$/u, "") + ".jpg", { type: "image/jpeg" });
  }
...
// auto-generated displayName pattern: ユーザー + 4 hex chars
...
export default function CommunityDiagnosisPage() {
    ...
    const load = async () => {
      ...
      const missing = allFields.filter((field) => {
        switch (field) {
          ...
          case "displayName":
            return isAutoGeneratedName(profile.displayName);
          case "gender":
  ...
  useEffect(() => {
    ...
    const save = async () => {
      ...
      for (const field of answeredFields) {
        switch (field) {
          case "avatar":
            profileUpdates.avatarIcon = avatarIconUrl;
            break;
          case "displayName":
  ...
  const handleAvatarPick = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith("image/")) {
      setAvatarUploadMessage("画像ファイルのみ選択できます。");
      return;
    }
    if (file.size > 5 * 1024 * 1024) {
      setAvatarUploadMessage("5MB以下の画像を選んでください。");
      return;
    }
    ...
    try {
      ...
      setAvatarUploadMessage("圧縮に失敗しました。別の画像をお試しください。");
    ...
    if (error) {
      setAvatarUploadMessage("アップロードに失敗しました。");
      setIsUploadingAvatar(false);
      return;
    }
          ...
          <div className="flex flex-col items-center gap-4">
            <div className="relative">
              {avatarIconUrl ? (
                ...
                <div className="grid h-24 w-24 place-items-center rounded-full bg-orange-50 text-4xl shadow-sm">
                  👤
                </div>
              )}
              {isUploadingAvatar && (
            ...
            {avatarUploadMessage && (
              <p className="text-center text-xs text-[var(--muted)]">{avatarUploadMessage}</p>
            )}
            <button
              type="button"
          ...
          <div>
            <div className="flex gap-2">
              <input
                ...
                onKeyDown={(event) => {
                  if (event.key === "Enter") { event.preventDefault(); addFavoriteArea(); }
                }}
                placeholder="例: 渋谷、梅田"
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {favoriteAreas.map((area) => (
                <button
                  key={area}
                  type="button"
                  onClick={() => setFavoriteAreas((prev) => prev.filter((item) => item !== area))}
          ...
          <div>
            <div className="flex gap-2">
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") { e.preventDefault(); handleSearchFrequentPlaces(); }
                }}
                ...
                {placeResults.map((place) => (
                  <div key={place.placeId} className="flex items-center gap-2 rounded-xl bg-white p-2">
                    ...
                    <button
                      ...
                      disabled={frequentPlaces.some((p) => p.placeId === place.placeId) || frequentPlaces.length >= 3}
            ...
            {frequentPlaces.length > 0 && (
              <div className="mt-3 space-y-2">
                {frequentPlaces.map((place) => (
                  <div key={place.placeId} className="flex items-center gap-2 rounded-xl border border-orange-100 bg-white p-2">
                    {place.photoUrl ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={place.photoUrl} alt={place.name} className="h-10 w-10 shrink-0 rounded-lg object-cover" />
                    ) : (
                    ...
                    <button
                      type="button"
                      onClick={() => setFrequentPlaces((prev) => prev.filter((p) => p.placeId !== place.placeId))}
          ...
          >
            ...
            <div className="mt-6 flex items-center gap-3">
              ...
              <button
                ...
                disabled={!isCurrentFieldAnswered || isUploadingAvatar}
